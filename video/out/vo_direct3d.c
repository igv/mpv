...
static bool change_d3d_backbuffer(d3d_priv *priv)
{
    ...
    const DWORD* shaders[NUM_SHADERS] = {
        ...
        [SHADER_SHARPEN] = (DWORD *)d3d_shader_sharpen,
    };
...
static uint32_t set_yuv_shader(d3d_priv *priv)
{
    if (FAILED(IDirect3DDevice9_SetPixelShader(priv->d3d_device,
...
static uint32_t d3d_draw_frame(d3d_priv *priv)
{
    ...
    if (priv->use_textures) {
        ...
        vertex_video tmptex_vb[] = {
            { 0.0f, 0.0f, 0.0f},
            { priv->vo->dwidth, 0.0f,  0.0f},
            { 0.0f, priv->vo->dheight, 0.0f},
            { priv->vo->dwidth, priv->vo->dheight, 0.0f}
        };
        ...
        for (n = 0; n < priv->plane_count; n++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                tmptex_vb[i].t[n][0] = vb[i].t[n][0];
                tmptex_vb[i].t[n][1] = vb[i].t[n][1];
            }
        }


        if (priv->use_shaders) {
            if(priv->opt_unsharp != 0.0f) {
                const float pixel_size[] = {1.0f / priv->vo->dwidth,
                                            1.0f / priv->vo->dheight};
                IDirect3DTexture9 *pTemporaryTexture;
                IDirect3DSurface9 *pTemporarySurface;
                D3DVIEWPORT9 oldVp;
                D3DSURFACE_DESC pDesc;

                IDirect3DDevice9_GetViewport(priv->d3d_device, &oldVp);
                IDirect3DSurface9_GetDesc(priv->d3d_backbuf, &pDesc);
                IDirect3DDevice9_CreateTexture(priv->d3d_device, pDesc.Width,
                                pDesc.Height, 1, D3DUSAGE_RENDERTARGET,
                                pDesc.Format, D3DPOOL_DEFAULT, &pTemporaryTexture,
                                NULL);

                IDirect3DTexture9_GetSurfaceLevel(pTemporaryTexture, 0,
                                                  &pTemporarySurface);

                IDirect3DDevice9_SetRenderTarget(priv->d3d_device, 0,
                                                 pTemporarySurface);

                set_yuv_shader(priv);

                IDirect3DDevice9_SetFVF(priv->d3d_device, D3DFVF_VIDEO_VERTEX);
                IDirect3DDevice9_DrawPrimitiveUP(priv->d3d_device,
                                 D3DPT_TRIANGLESTRIP, 2, &tmptex_vb[0],
                                 sizeof(vertex_video));

                IDirect3DDevice9_SetRenderTarget(priv->d3d_device, 0,
                                                 priv->d3d_backbuf);
                IDirect3DDevice9_SetTexture(priv->d3d_device, 0,
                                (IDirect3DBaseTexture9 *)pTemporaryTexture);
                IDirect3DDevice9_SetViewport(priv->d3d_device, &oldVp);

                IDirect3DDevice9_SetPixelShader(priv->d3d_device,
                                            priv->pixel_shaders[SHADER_SHARPEN]);
                IDirect3DDevice9_SetPixelShaderConstantF(priv->d3d_device, 0,
                                                 &pixel_size[0], 1);
                IDirect3DDevice9_SetPixelShaderConstantF(priv->d3d_device, 1,
                                                 &priv->opt_unsharp, 1);

                IDirect3DSurface9_Release(pTemporarySurface);
                IDirect3DTexture9_Release(pTemporaryTexture);
            } else {
                set_yuv_shader(priv);
            }
        }
...
static const struct m_option opts[] = {
    OPT_FLOAT("sharpen", opt_unsharp, 0),
